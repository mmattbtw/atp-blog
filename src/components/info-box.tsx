...
export default function InfoBox() {
  ...
  const [blueskyPost, setBlueskyPost] = useState<AppBskyFeedPost.Main | null>(
    null,
  );
    ...
    const fetchData = async () => {
      try {
        ...
        if (cars.records.length > 0) {
          setCarInfo(cars.records[0].value as CarRecord);
        }
        ...
        try {
          ...
          if (nonReplyPost) {
            setBlueskyPost(nonReplyPost.value as AppBskyFeedPost.Main);
        ...
        try {
          ...
          if (teal.records.length > 0) {
            setTealPlay(teal.records[0].value as TealPlayRecord);
          }
        } catch {
        ...
        try {
          ...
          if (now.records.length > 0) {
            setNowEntry(now.records[0].value as NowRecord);
          }
        } catch {
